...
/datum/game_mode/proc/get_alive_non_antagonsist_players_for_role(role) // :shrug:, Mostly used for ex-communicated tator
	var/list/candidates = list()

	for(var/mob/living/carbon/human/player in GLOB.player_list)
		if(player.client && is_station_level(player.z))
			if(role in player.client.prefs.be_special)
				if(!jobban_isbanned(player, (ROLE_SYNDICATE)) && !QDELETED(player))
