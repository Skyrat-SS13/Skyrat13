/obj/machinery/computer/apc_control
	...
	var/list/the_Filters //For sorting the results
	var/checking_logs = 0
	var/list/logs
	var/authenticated = 0
	var/auth_id = "\[NULL\]"
...
/obj/machinery/computer/apc_control/Initialize()
	...
	the_Filters = list("Name" = null, "Charge Above" = null, "Charge Below" = null, "Responsive" = null)
	..()
...
/obj/machinery/computer/apc_control/interact(mob/living/user)
	...
	if(authenticated)
		if(!checking_logs)
			...
			dat += "<i>the_Filters</i><br>"
			dat += "<b>Name:</b> <a href='?src=\ref[src];name_filter=1'>[the_Filters["Name"] ? the_Filters["Name"] : "None set"]</a><br>"
			dat += "<b>Charge:</b> <a href='?src=\ref[src];above_filter=1'>\>[the_Filters["Charge Above"] ? the_Filters["Charge Above"] : "NaN"]%</a> and <a href='?src=\ref[src];below_filter=1'>\<[the_Filters["Charge Below"] ? the_Filters["Charge Below"] : "NaN"]%</a><br>"
			dat += "<b>Accessible:</b> <a href='?src=\ref[src];access_filter=1'>[the_Filters["Responsive"] ? "Non-Responsive Only" : "All"]</a><br><br>"
			for(var/A in apcs)
				var/obj/machinery/power/apc/APC = apcs[A]
				if(the_Filters["Name"] && !findtext(APC.name, the_Filters["Name"]) && !findtext(APC.area.name, the_Filters["Name"]))
					continue
				if(the_Filters["Charge Above"] && (APC.cell.charge / APC.cell.maxcharge) < the_Filters["Charge Above"] / 100)
					continue
				if(the_Filters["Charge Below"] && (APC.cell.charge / APC.cell.maxcharge) > the_Filters["Charge Below"] / 100)
					continue
				if(the_Filters["Responsive"] && !APC.aidisabled)
					continue
...
/obj/machinery/computer/apc_control/Topic(href, href_list)
	...
	if(href_list["above_filter"])
		...
		playsound(src, 'sound/machines/terminal_prompt_confirm.ogg', 50, 0)
		the_Filters["Charge Above"] = new_filter
	if(href_list["below_filter"])
		...
		if(new_filter)
			new_filter = Clamp(new_filter, 0, 100)
		playsound(src, 'sound/machines/terminal_prompt_confirm.ogg', 50, 0)
		the_Filters["Charge Below"] = new_filter
	if(href_list["access_filter"])
		if(isnull(the_Filters["Responsive"]))
			the_Filters["Responsive"] = 1
			log_activity("sorted by non-responsive APCs only")
		else
			the_Filters["Responsive"] = !the_Filters["Responsive"]
