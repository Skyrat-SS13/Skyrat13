...
/proc/velvetspeech(message, mob/living/user, base_multiplier = 1, message_admins = FALSE, debug = FALSE)
	...
	//Mixables
	var/static/regex/enthral_words = regex("relax|obey|love|serve|so easy|ara ara|повинуйся|расслабься|служи")
	var/static/regex/reward_words = regex("good boy|good girl|good pet|good job|splendid|jolly good|bloody brilliant|хороший мальчик|хорошая девочка|хорошая работа|хороший питомец")
	var/static/regex/punish_words = regex("bad boy|bad girl|bad pet|bad job|spot of bother|gone and done it now|blast it|buggered it up|плохой мальчик|плохая девочка|плохая работа|плохой питомец")
	//phase 0
	var/static/regex/saymyname_words = regex("say my name|who am i|whoami|кто я|скажи мое имя")
	var/static/regex/wakeup_words = regex("revert|awaken|snap|attention|внимание|обратись|проснись")
	//phase1
	var/static/regex/petstatus_words = regex("how are you|what is your status|are you okay|ты как|как ты|ты в норме|твой статус")
	var/static/regex/silence_words = regex("shut up|silence|be silent|ssh|quiet|hush|тихо|заткнись|тишина")
	var/static/regex/speak_words = regex("talk to me|speak|говори")
	var/static/regex/antiresist_words = regex("unable to resist|give in|stop being difficult|сдавайся|не сопротивляйся")//useful if you think your target is resisting a lot
	var/static/regex/resist_words = regex("resist|snap out of it|fight|борись|сопротивляйся")//useful if two enthrallers are fighting
	var/static/regex/forget_words = regex("forget|muddled|awake and forget|забудь|нет гипнозу")
	var/static/regex/attract_words = regex("come here|come to me|get over here|attract|иди сюда|иди ко мне|подойди")
	//phase 2
	var/static/regex/orgasm_words = regex("cum|orgasm|climax|squirt|heyo|оргазм|кончи|климакс") //wah, lewd
	var/static/regex/awoo_words = regex("howl|awoo|bark|ауу|гав")
	var/static/regex/nya_words = regex("nya|meow|mewl|мяу|ня")
	var/static/regex/sleep_words = regex("sleep|slumber|rest|спи|поспи|отдохни")
	var/static/regex/strip_words = regex("strip|derobe|nude|at ease|suit off|разденься|сними одежду|оголись")
	var/static/regex/walk_words = regex("slow down|walk|шагай")
	var/static/regex/run_words = regex("run|speed up|беги")
	var/static/regex/liedown_words = regex("lie down|ляг|ложись")
	var/static/regex/knockdown_words = regex("drop|fall|trip|knockdown|kneel|army crawl|упади|крадись|подскользнись")
	//phase 3
	var/static/regex/statecustom_words = regex("state triggers|state your triggers|ключевые слова")
	var/static/regex/custom_words = regex("new trigger|listen to me|новое слово|слушай меня")
	...
	var/static/regex/instill_words = regex("feel|entice|overwhel|почувствуй")
	var/static/regex/recognise_words = regex("recognise me|did you miss me?|вспомни меня|ты скучал?|ты скучала?")
	var/static/regex/objective_words = regex("new objective|obey this command|unable to resist|compulsed|word from hq|новая задача|исполни|повинуйся команде")
	var/static/regex/heal_words = regex("live|heal|survive|mend|life|pets never die|heroes never die|живи|вылечись|не умирай|питомцы не умирают")
	var/static/regex/stun_words = regex("stop|wait|stand still|hold on|halt|стоять|стой смирно|стой") 
	var/static/regex/hallucinate_words = regex("get high|hallucinate|trip balls|галлюцинируй")
	var/static/regex/hot_words = regex("heat|hot|hell|горячо|жар|ад")
	var/static/regex/cold_words = regex("cold|cool down|chill|freeze|остынь|охладись|мороз")
	var/static/regex/getup_words = regex("get up|hop to it|вставай")
	var/static/regex/pacify_words = regex("docile|complaisant|friendly|pacifist|пацифист|дружелюбие|мир")
	var/static/regex/charge_words = regex("charge|oorah|attack|атакуй|заряди")
