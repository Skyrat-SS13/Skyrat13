		...
		if(!currentbones.len)
			if(prob(20))
				for(var/obj/item/organ/O in getorganszone(currentzone))
					src.adjustOrganLoss(O, rand(3,5))
				src.apply_damage(0.25, BRUTE, currentzone, FALSE, TRUE)
				if(prob(25))
					to_chat(H, "<span class='warning'>Your [currentzone] is squishy and hurts like fucking hell! What the fuck?!</span>")
		else
					...
					if(BONE_DAM_THRESHOLD_LOW to (BONE_DAM_THRESHOLD_MEDIUM - 1))
						if(prob(2))
							for(var/obj/item/organ/ourorgan in getorganszone(currentzone))
								if(ourorgan.zone = currentzone && ourorgan != B)
									src.adjustOrganLoss(ourorgan, 1)
							to_chat(H, "<span class='warning'>Your [currentzone] feels sore.</span>")
					if(BONE_DAM_THRESHOLD_MEDIUM to (BONE_DAM_THRESHOLD_HIGH - 1))
						if(prob(4))
							for(var/obj/item/organ/ourorgan in getorganszone(currentzone))
								if(ourorgan.zone = currentzone && ourorgan != B)
									src.adjustOrganLoss(ourorgan, 2)
							to_chat(H, "<span class='warning'>Your [currentzone] hurts!</span>")
					if(BONE_DAM_THRESHOLD_HIGH to (BONE_DAM_THRESHOLD_HIGHEST - 1))
						if(prob(7))
							for(var/obj/item/organ/ourorgan in getorganszone(currentzone))
								if(ourorgan.zone = currentzone && ourorgan != B)
									src.adjustOrganLoss(ourorgan, 2)
							to_chat(H, "<span class='big bold'>Your [currentzone] hurts a lot!</span>")
					if(BONE_DAM_THRESHOLD_HIGHEST)]
						if(prob(12))
							for(var/obj/item/organ/ourorgan in getorganszone(currentzone))
								if(ourorgan.zone = currentzone && ourorgan != B)
									src.adjustOrganLoss(ourorgan, rand(2,3)
							src.apply_damage(0.5, BRUTE, currentzone, FALSE, TRUE)
							to_chat(H, "<span class='big bold'>Your [currentzone] hurts like hell!</span>")
					else
						if(prob(12))
							for(var/obj/item/organ/ourorgan in getorganszone(currentzone))
								if(ourorgan.zone = currentzone && ourorgan != B)
