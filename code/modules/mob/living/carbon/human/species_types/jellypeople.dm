...
/datum/action/innate/slime_change/proc/change_form()
	...
	if(select_alteration == "Body Color")
		...
		var/new_length = input(owner, "Penis length in centimeters:\n([min_D]-[max_D])", "Genital Alteration") as num|null //new era -- fuck the imperial system
		if(new_length)
			H.dna.features["cock_length"] = clamp(round(CM2IN(new_length)), min_D, max_D) //new era end
		H.update_genitals()
		H.apply_overlay()
		H.give_genital(/obj/item/organ/genital/testicles)
		H.give_genital(/obj/item/organ/genital/penis)
