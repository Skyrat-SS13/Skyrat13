		...
		if(!lying && !buckled)
			if(loc == NewLoc)
				...
				if(S.bloody_shoes && S.bloody_shoes[S.blood_state])
					...
					if(oldFP && (oldFP.blood_state == S.blood_state && oldFP.color == bloodtype_to_color(S.last_blood_color)))
						return
					...
					if(S.last_bloodtype)
						FP.blood_DNA += list("color" = list(S.last_blood_DNA = S.last_blood_color), S.last_blood_DNA = S.last_bloodtype)
					FP.update_icon()
					update_inv_shoes()
