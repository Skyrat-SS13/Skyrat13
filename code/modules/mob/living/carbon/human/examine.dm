...
//END OF CIT CHANGES
	...
	msg += length(list) ? "<br>[show_descs.Join("<br>")]<br>" : "<br>"
	...
	if(LAZYLEN(dna?.species?.descriptors))
		if(user == src)
			for(var/entry in dna.species.descriptors)
				...
				. += "<b><span class='info'>[descriptor.get_third_person_message_start(src)] [descriptor.get_standalone_value_descriptor(descriptor.current_value)].</span></b><BR>"
		else
			for(var/entry in dna.species.descriptors)
				var/datum/mob_descriptor/descriptor = dna.species.descriptors[entry]
				. += "<b><span class='info'>[descriptor.get_comparative_value_descriptor(src, user, descriptor.current_value)]</span></b><BR>"
//
