...
/obj/item/organ/genital/penis/update_size(modified = FALSE)
	...
	if(owner)
		if (round(length) > round(prev_length))
			//new era -- fuck the imperial system
			to_chat(owner, "<span class='warning'>Your [pick(GLOB.dick_nouns)] [pick("swells up to", "flourishes into", "expands into", "bursts forth into", "grows eagerly into", "amplifys into")] a [uppertext(round(IN2CM(length)))]cm penis.</b></span>")
		else if ((round(length) < round(prev_length)) && (length > 0.5))
			to_chat(owner, "<span class='warning'>Your [pick(GLOB.dick_nouns)] [pick("shrinks down to", "decreases into", "diminishes into", "deflates into", "shrivels regretfully into", "contracts into")] a [uppertext(round(IN2CM(length)))]cm penis.</b></span>")
			//new era end
...
/obj/item/organ/genital/penis/update_appearance()
	...
	//new era -- fuck the imperial system
	desc = "You see [aroused_state ? "an erect" : "a flaccid"] [lowershape] [name]. You estimate it's about [round(IN2CM(length), 0.25)]cm long and [round(IN2CM(diameter), 0.25)]cm in diameter."
