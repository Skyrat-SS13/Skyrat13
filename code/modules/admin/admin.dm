...
/datum/admins/proc/show_player_panel(mob/M in GLOB.mob_list)
	...
	var/collarline = "[jobban_isbanned(M, COLLARBAN)?"Remove Collar Ban":"Collar Ban"]" // SKYRAT ADDITION -- BEGIN
	if(ishuman(M))
		var/mob/living/carbon/human/C = M
		if(!istype(C.wear_neck, COLLARITEM) && jobban_isbanned(M, COLLARBAN))
			collarline = "FIX COLLAR"
	body += "<A href='?_src_=holder;[HrefToken()];collarban=[REF(M)]'>[collarline]</A> | " // SKYRAT ADDITION -- END
	var/rm = REF(M)
