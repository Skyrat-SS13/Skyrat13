...
/client/proc/play_web_sound()
	...
	if(istext(web_sound_input))
		...
		if(length(web_sound_input))
			...
			var/cmd_string = "[ytdl] --format \"bestaudio\[ext=mp3]/best\[ext=mp4]\[height\<=360]/bestaudio\[ext=m4a]/bestaudio\[ext=aac]\" --dump-single-json --no-playlist -- \"[shell_scrubbed_input]\""
			world.log << cmd_string
			var/list/output = world.shelleo("[cmd_string]")
