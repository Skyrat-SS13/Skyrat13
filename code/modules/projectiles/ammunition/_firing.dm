	...
	//
		if(distro) //We have to spread a pixel-precision bullet. throw_proj was called before so angles should exist by now...
			if(randomspread)
				...
				//it wasnt that smart so ryll changed it i guess
				spread = round(1 - 0.5) * distro
				//
		if(!throw_proj(target, targloc, user, params, spread))
			return FALSE //skyrat edit all my homies hate 0 being used as a boolean
	/* skyrat edit the code is still cum
		if(i > 1)
	*/
	//skyrat edit
	else
