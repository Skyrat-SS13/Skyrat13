...
/datum/action/vampire/proc/CheckCanPayCost(display_error)
	if(!owner || !owner.mind)
		return FALSE
	// Cooldown?
...
/datum/action/vampire/proc/StartCooldown()
	set waitfor = FALSE
	// Alpha Out
...
/datum/action/vampire/proc/CheckCanDeactivate(display_error)
	return TRUE

/datum/action/vampire/UpdateButtonIcon(force = FALSE)
	background_icon_state = active? background_icon_state_on : background_icon_state_off
	..()//UpdateButtonIcon()


/datum/action/vampire/proc/PayCost()
...
/datum/action/vampire/proc/DeactivatePower(mob/living/user = owner, mob/living/target)
	active = FALSE
	UpdateButtonIcon()
	StartCooldown()

/datum/action/vampire/proc/ContinueActive(mob/living/user, mob/living/target) // Used by loops to make sure this power can stay active.
	return active && user && (!warn_constant_cost || user.blood_volume > 0)

/datum/action/vampire/proc/RemoveAfterUse()
	// Un-Learn Me! (GO HOME
	var/datum/antagonist/vampire/vampiredatum = owner.mind.has_antag_datum(/datum/antagonist/vampire)
	if (istype(vampiredatum))
		vampiredatum.powers -= src
	Remove(owner)
...
/datum/action/vampire/passive

/datum/action/vampire/passive/New()
	// REMOVED: DO NOTHBING!
	..()
...
/datum/action/vampire/passive/Destroy()
	if(owner)
		Remove(owner)
	target = null
...
/datum/action/vampire/targeted
	...
	var/obj/effect/proc_holder/vampire/bs_proc_holder
...
/datum/action/vampire/targeted/New(Target)
...
/datum/action/vampire/targeted/Trigger()
	if(active && CheckCanDeactivate(TRUE))
...
/datum/action/vampire/targeted/CheckCanUse(display_error)
	. = ..()
	if(!.)
		return
...
/datum/action/vampire/targeted/DeactivatePower(mob/living/user = owner, mob/living/target)
	// Don't run ..(), we don't want to engage the cooldown until we USE this power!
	active = FALSE
	UpdateButtonIcon()

/datum/action/vampire/targeted/proc/DeactivateRangedAbility()
	// Only Turned off when CLICK is disabled...aka, when you successfully clicked (or
	bs_proc_holder.remove_ranged_ability()

// Check if target is VALID (wall, turf, or character?)
/datum/action/vampire/targeted/proc/CheckValidTarget(atom/A)
	return FALSE // FALSE targets nothing.

// Check if valid target meets conditions
/datum/action/vampire/targeted/proc/CheckCanTarget(atom/A, display_error)
	// Out of Range
...
// Click Target
/datum/action/vampire/targeted/proc/ClickWithPower(atom/A)
...
/datum/action/vampire/targeted/proc/FireTargetedPower(atom/A)
	// Like ActivatePower, but specific to Targeted (and takes an atom input). We don't use ActivatePower for targeted.

/datum/action/vampire/targeted/proc/PowerActivatedSuccessfully()
...
/datum/action/vampire/targeted/ContinueActive(mob/living/user, mob/living/target) // Used by loops to make sure this power can stay active.
	return ..()
// Target Proc Holder
/obj/effect/proc_holder/vampire
	var/datum/action/vampire/targeted/linked_power

/obj/effect/proc_holder/vampire/remove_ranged_ability(msg)
	..()
	linked_power.DeactivatePower()

/obj/effect/proc_holder/vampire/InterceptClickOn(mob/living/caller, params, atom/A)
