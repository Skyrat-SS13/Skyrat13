...
/datum/component/tackler/proc/splat(mob/living/carbon/user, atom/hit)
	...
	switch(oopsie)
		if(99 to INFINITY)
			...
			user.visible_message("<span class='danger'>[user] slams face-first into [hit] at an awkward angle, really fucking [user.p_their()] head up! Holy shit!</span>", "<span class='userdanger'>You slam face-first into [hit] at an awkward angle, really fucking your head up! Holy shit!</span>")
			user.adjustStaminaLoss(100)
			user.adjustBruteLoss(60)
			user.Unconscious(100)
			user.gain_trauma_type(BRAIN_TRAUMA_SEVERE,TRAUMA_RESILIENCE_SURGERY)
