...
/datum/component/butchering/proc/onItemAttack(obj/item/source, mob/living/M, mob/living/user)
	...
	if(ishuman(M) && source.force && source.get_sharpness())
		...
		if((H.nervous_system_failure() || H.is_asystole() || (user.pulling == H && user.grab_state >= GRAB_NECK) || H.IsSleeping()) && user.zone_selected == BODY_ZONE_PRECISE_MOUTH) // Only sleeping, neck grabbed, or crit, can be sliced.
