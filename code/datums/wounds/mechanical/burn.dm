	...
	switch(power)
		if(1)
			if(limb.body_zone in list(BODY_ZONE_L_ARM, BODY_ZONE_PRECISE_L_HAND, BODY_ZONE_R_ARM, BODY_ZONE_PRECISE_R_HAND))
				to_chat(victim, "<span class='danger'>Your [limb.name] flops around wildly!</span>")
				...
				if(istype(A))
					...
					H.ClickOn(A)
					if(H.back == A)
						A.storage_contents_dump_act(victim.loc, victim)
				...
				to_chat(victim, "<span class='danger'>Your [limb.name] moves on it's own!</span>")
				victim.Move(get_step(victim, pick(GLOB.alldirs)))
			else if(limb.body_zone in list(BODY_ZONE_HEAD, BODY_ZONE_CHEST))
				to_chat(victim, "<span class='danger'>Your [limb.name] fails to receive input!</span>")
				victim.Paralyze(10, TRUE)
		if(2)
			if(limb.body_zone in list(BODY_ZONE_L_ARM, BODY_ZONE_PRECISE_L_HAND, BODY_ZONE_R_ARM, BODY_ZONE_PRECISE_R_HAND))
				to_chat(victim, "<span class='userdanger'>You lose control of your [limb.name]!</span>")
				var/list/targets = list()
				var/list/mobs = list()
				var/list/objs = list()
				var/list/turfs = list()
				for(var/atom/A in (view(1, victim) - victim))
					if(!isarea(A))
						targets += A
					if(ismob(A))
						mobs += A
				...
				to_chat(victim, "<span class='userdanger'>Your [limb.name] gets stuck in a while loop!</span>")
				var/list/turfs = list()
				for(var/turf/T in (view(5, victim) - victim.loc))
					turfs += T
				var/turf/walkie = pick(turfs)
				if(istype(walkie))
					victim.Paralyze(7 * get_dist(victim, walkie))
					walk_to(victim, walkie, 0, 3, 0)
			else if(limb.body_zone in list(BODY_ZONE_HEAD, BODY_ZONE_CHEST))
				to_chat(victim, "<span class='warning'>Your [limb.name] runtimes at line [rand(1,1000)] of [pick("life.dm", "bodypart.dm", "mob_defense.dm", "AI.dm", "[lowertext(H.dna.species.name)].dm")]!</span>")
				victim.DefaultCombatKnockdown(35)
		if(3)
			if(limb.body_zone in list(BODY_ZONE_L_ARM, BODY_ZONE_PRECISE_L_HAND, BODY_ZONE_R_ARM, BODY_ZONE_PRECISE_R_HAND))
				to_chat(victim, "<span class='warning'>Your [limb.name] fails to compile!</span>")
				victim.a_intent_change(INTENT_HARM)
				victim.ClickOn(victim)
				victim.changeNext_move(CLICK_CD_MELEE * 4)
				if(prob(50))
					victim.changeNext_move(0)
					victim.ClickOn(victim)
			else if(limb.body_zone in list(BODY_ZONE_L_LEG, BODY_ZONE_PRECISE_L_FOOT, BODY_ZONE_R_LEG, BODY_ZONE_PRECISE_R_FOOT, BODY_ZONE_PRECISE_GROIN))
				to_chat(victim, "<span class='warning'>Your [limb.name] locks up!</span>")
				victim.lay_down()
				victim.Paralyze(25)
				victim.adjustStaminaLoss(25)
				victim.adjustStaminaLossBuffered(25)
			else if(limb.body_zone in list(BODY_ZONE_HEAD, BODY_ZONE_CHEST))
				to_chat(victim, "<span class='warning'>Your [limb.name] divides by zero!</span>")
