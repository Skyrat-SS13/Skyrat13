...
#define STANDARD_PART_LEVEL_LATHE_COEFFICIENT(level)		clamp(1 - (level * 0.125), 0.125, 1) //SKYRAT CHANGE - Reverted costs to pre-balance changes
...
#define STANDARD_PART_LEVEL_ORE_COEFFICIENT(level)			clamp(1 + (level * 0.2), 1, 10) //SKYRAT CHANGE - Reverted costs to pre-balance changes
