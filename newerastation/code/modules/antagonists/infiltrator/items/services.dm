...
/obj/item/service/ion/attack_self(mob/user)
	if(GLOB.next_button_push > world.time)
		return
	priority_announce("Ion storm detected near the station. Please check all AI-controlled equipment for errors.", "Anomaly Alert", get_announcer_sound("ionstorm"))
...
/obj/item/service/meteor/attack_self(mob/user)
	if(GLOB.next_button_push > world.time)
		return
	priority_announce("Meteors have been detected on collision course with the station.", "Meteor Alert", get_announcer_sound("meteors"))
