			...
			if(M && M.client)
				...
				if(M_turf && M_turf.z == src.z)
					...
					if(dist <= 7) //source of sound very close
						M.playsound_local(src, 'modular_skyrat/master_files/sound/effects/horror_scream.ogg', 80, 1, frequency, falloff = 2)
					else
...
/mob/living/simple_animal/hostile/true_changeling/death()
	...
	if(stored_changeling && mind)
		...
		stored_changeling.Paralyze(10 SECONDS) //Make them helpless for 10
