...
/obj/item/projectile/energy/bs_electrode/on_hit(atom/target, blocked = FALSE)
	. = ..()
...
/obj/item/projectile/energy/bs_electrode/on_range() //to ensure the bolt sparks when it reaches the end of its range if it didn't hit a target yet
	do_sparks(1, TRUE, src)
	..()
