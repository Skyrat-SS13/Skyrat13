...
/obj/item/bodypart/proc/on_life()
	...
	if(pain_heal_tick && pain_dam > DAMAGE_PRECISION)
		if(heal_damage(pain = (pain_heal_tick * (owner?.lying ? pain_heal_rest_multiplier : 1) * (owner.mind?.mob_stats[/datum/stats/end] ? (owner.mind.mob_stats[/datum/stats/end]/10) : 1)), only_robotic = FALSE, only_organic = FALSE, updating_health = FALSE))
			. |= BODYPART_LIFE_UPDATE_HEALTH
