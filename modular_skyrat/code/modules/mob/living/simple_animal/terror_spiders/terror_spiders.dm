...
/mob/living/simple_animal/hostile/poison/terror_spider/terror_spider_check()
	return TRUE
...
/mob/living/simple_animal/hostile/poison/terror_spider/proc/CheckFaction()
	if(faction.len != 2 || (!("terrorspiders" in faction)))
		to_chat(src, "<span class='userdanger'>Your connection to the hive mind has been severed!</span>")
		var/turf/t = get_turf(src)
		if(istype(t))
			log_runtime(EXCEPTION("Terror spider with incorrect faction list at: [loc] ([t.x],[t.y],[t.z]) ([loc.type])"))
		gib()
