...
/datum/species/synth/spec_life(mob/living/carbon/human/H)
	...
	if((actualhealth < disguise_fail_health) && isdisguised)
		unassume_disguise(H)
		H.visible_message("<span class='danger'>[user]'s disguise falls apart!</span>", "<span class='userdanger'>Your diguise falls apart!</span>")
		handle_hair(H)
		handle_body(H)
		...
		H.visible_message("<span class='warning'>[user] morphs their appearance to that of [fake_species.name].</span>", "<span class='notice'>You morph your appearance to that of [fake_species.name].</span>")
		handle_hair(H)
		handle_body(H)
		handle_mutant_bodyparts(H)
		H.updatehealth()
