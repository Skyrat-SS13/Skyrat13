/obj/item/organ/regenerative_core/afterattack(atom/target, mob/user, proximity_flag)
	...
	if(proximity_flag && ishuman(target) && is_station_level(target.z))
		...
		if(!inert)
			...
			to_chat(user, "<span class='notice'>[src] has decayed and can no longer be used to heal.</span>")
	if(istype(target, /mob/living/simple_animal/hostile/megafauna/bubblegum) && !istype(target, /mob/living/simple_animal/hostile/megafauna/bubblegum/hard) && (target.stat != DEAD))
		...
		qdel(target)
		qdel(src)
	if(istype(target, /mob/living/simple_animal/hostile/megafauna/dragon) && !istype(target, /mob/living/simple_animal/hostile/megafauna/dragon/hard) && (target.stat != DEAD))
		new /mob/living/simple_animal/hostile/megafauna/dragon/hard(target.loc)
		visible_message("[user] turned [target] into an enraged [target]!")
		qdel(target)
		qdel(src)
	if(istype(target, /mob/living/simple_animal/hostile/megafauna/legion) && !istype(target, /mob/living/simple_animal/hostile/megafauna/legion) && (target.stat != DEAD))
