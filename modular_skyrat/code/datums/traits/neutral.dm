...
/datum/quirk/synthetic/add()
	...
	if(istype(H))
		if(!(H.dna.species.type in blacklistedspecies))
			...
			return TRUE
	to_chat(H, "<span class='warning'>Your species or mob type is blacklisted from being a synth. Your synth quirk will be removed and your species will not be changed.</span>")
