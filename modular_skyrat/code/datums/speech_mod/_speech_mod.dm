...
/mob/living/carbon/proc/enable_speech_mod(datumtype)
	for(var/datum/speech_mod/S in speech_mods)
		if(istype(S,datumtype))
			return
	var/datum/speech_mod/speech = new datumtype
...
/mob/living/carbon/proc/disable_speech_mod(datumtype)
	for(var/datum/speech_mod/S in speech_mods)
		if(istype(S,datumtype))
			S.remove_speech_mod()
			qdel(S)
...
/datum/speech_mod/proc/add_speech_mod(mob/living/carbon/M)
	affected_mob = M
