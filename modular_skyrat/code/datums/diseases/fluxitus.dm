/datum/disease/fluxitus
	...
	var/new_scream = list('modular_skyrat/sound/roblox/OOF.wav')
...
/datum/disease/fluxitus/stage_act()
	...
	switch(stage)
		...
		if(2)
			...
			if(prob(3) && affected_mob.getOrganLoss(ORGAN_SLOT_BRAIN)<=98)
		...
		if(4)
			...
			if(prob(20))
				affected_mob.emote("scream")
			if(prob(10) && affected_mob.getOrganLoss(ORGAN_SLOT_BRAIN)<=98)
				affected_mob.adjustOrganLoss(ORGAN_SLOT_BRAIN, 5)
				affected_mob.updatehealth()
		...
		if(5) //Better have an epipen to hand or you is a ded boi
			if(prob(50))
				affected_mob.emote("scream")
			if(prob(30) && affected_mob.getOrganLoss(ORGAN_SLOT_BRAIN)<=98)
