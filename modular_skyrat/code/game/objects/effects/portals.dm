...
//had to port all this portal shit from tg
/obj/effect/portal
	name = "portal"
...
/obj/effect/portal/unlink_atmos()
...
/obj/effect/portal/teleport(atom/movable/M, force = FALSE)
	if(!force && (!istype(M) || iseffect(M) || (ismecha(M) && !mech_sized) || (!isobj(M) && !ismob(M)))) //Things that shouldn't teleport.
		return
	var/turf/real_target = get_link_target_turf()
	if(!istype(real_target))
...
/obj/effect/portal/get_link_target_turf()
