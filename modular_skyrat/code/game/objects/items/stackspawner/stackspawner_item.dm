/obj/item/stackspawner
	...
	icon = 'modular_skyrat/code/game/objects/items/stackspawner/stackspawner.dmi'
...
/obj/item/stackspawner/attack_self(mob/user)
	...
	if(!material_list)
		material_list = list(
			...
			"Delivery Paper"			= image(icon = icon, icon_state = "delivery"),
	...
	switch(choice)
		if("Metal")
			itemstack = /obj/item/stack/sheet/metal
		if("Glass")
		...
		if("Brass")
			itemstack = /obj/item/stack/tile/brass
		if("Runed")
		...
		if("Delivery Paper")
			itemstack = /obj/item/stack/packageWrap
		if("Bone")
