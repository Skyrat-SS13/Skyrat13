...
/obj/structure/chair/noose/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/wirecutters) || W.sharpness == IS_SHARP || W.sharpness == IS_SHARP_ACCURATE)
...
/obj/structure/chair/noose/user_buckle_mob(mob/living/carbon/human/M, mob/user)
	...
	if(do_mob(user, M, user == M ? 0:200))
		if(buckle_mob(M))
			...
			playsound(user.loc, 'modular_skyrat/sound/effects/noosed.ogg', 50, 1, -1)
	...
	for(var/m in buckled_mobs)
		...
		if(buckled_mob.mob_has_gravity())
			...
			if(prob(20))
				...
				playsound(buckled_mob.loc, 'modular_skyrat/sound/effects/noose_idle.ogg', 30, 1, -3)
