...
/datum/component/construction/mecha/powerarmor
	result = /obj/item/mecha_parts/part/chassis/powerarmor
	base_icon = "ripley"
	steps = list(
		//1
		list(
...
/datum/component/construction/mecha/powerarmor/custom_action(obj/item/I, mob/living/user, diff)
	...
	switch(index)
		if(1)
			user.visible_message("[user] connects [parent]'s parts to the chassis.", "<span class='notice'>You connect [parent]'s parts to the chassis.</span>")
		if(2)
			if(diff==FORWARD)
				user.visible_message("[user] wrenches[parent]'s parts.", "<span class='notice'>You wrench [parent]'s parts.</span>")
			else
				user.visible_message("[user] disconnects [parent]'s parts.", "<span class='notice'>You disconnect [parent]'s parts.</span>")
		if(3)
			if(diff==FORWARD)
				user.visible_message("[user] welds [parent]'s parts to the chassis.", "<span class='notice'>You weld [parent]'s parts to the chassis.</span>")
			else
		...
		if(8)
			if(diff==FORWARD)
				user.visible_message("[user] wires the [parent]'s chassis.", "<span class='notice'>You wire the [parent]'s chassis.</span>")
			else
